...
public class DemoBattleController {
    ...
    public ModelAndView db1vs1Select(ModelAndView mv) {
        ...
        if (session.getAttribute("login_user") != null) {
            List<PcEntity> up = pcrepository.findByUserAndDeleteFlag((User) session.getAttribute("login_user"), 0);
            mv.addObject("up", up);

            List<PcEntityLike> likes = pcLikerepository.findByUser((User) session.getAttribute("login_user"));
            List<PcEntity> lp = new ArrayList<PcEntity>();
            for (int i = 0; i < likes.size(); i++) {
                if (likes.get(i).getPc_entity().getDeleteFlag() == 0) {
                    lp.add(likes.get(i).getPc_entity());
                }
            }
    ...
    public ModelAndView dbLog(@RequestParam(name = "pc1") Integer p1, @RequestParam(name = "pc2") Integer p2,
            ModelAndView mv) {
        ...
        while (first.getHp() > 0 && latter.getHp() > 0 && round < 100) {
            ...
            if (roll < 6) { //クリティカルだった場合-------------------------------------
                log.add(first.getName() + " : " + "1D100<=" + first.attackSkill() + "【 " + first.attackName() + " 】"
                        + "(1D100<=" + first.attackSkill() + ")＞ " + roll + " ＞ "
                        + "<span style='color:#4682b4;'>クリティカル！</span>");
                ...
                log.add(first.getName() + " : " + "1D100<=" + first.attackSkill() + "【 " + first.attackName() + " 】"
                        + "(1D100<=" + first.attackSkill() + ")＞ " + roll + " ＞ " + "成功！");
                ...
                //回避
                if (roll < 6) { //回避クリティカル
                    log.add(latter.getName() + " : " + "1D100<=" + latter.getAvoidance() + "【 回避 】(1D100<="
                            + latter.getAvoidance() + ")＞ " + roll + " ＞ "
                            + "<span style='color:#4682b4;'>クリティカル！</span>");
                    ...
                    log.add(latter.getName() + " : " + "1D100<=" + latter.getAvoidance() + "【 回避 】(1D100<="
                            + latter.getAvoidance() + ")＞ " + roll + " ＞ "
                            + "<span style='color:#dc143c;'>ファンブル！</span>");
                    log.add("▼" + first.getName() + "の連続攻撃！");
                    DemoBattle.attake(first, latter, firstDb, log);
                    DemoBattle.attake(first, latter, firstDb, log);
                } else if (latter.getAvoidance() >= roll) {
                    log.add(latter.getName() + " : " + "1D100<=" + latter.getAvoidance() + "【 回避 】(1D100<="
                            + latter.getAvoidance() + ")＞ " + roll + " ＞ " + "回避成功！");
                } else {
                    log.add(latter.getName() + " : " + "1D100<=" + latter.getAvoidance() + "【 回避 】(1D100<="
                            + latter.getAvoidance() + ")＞ " + roll + " ＞ " + "回避失敗");
                    //攻撃～気絶判定--------------------------------------------------------
                    DemoBattle.attake(first, latter, firstDb, log);
                }
                ...
                log.add(first.getName() + " : " + "1D100<=" + first.attackSkill() + "【 " + first.attackName() + " 】"
                        + "(1D100<=" + first.attackSkill() + ")＞ " + roll + " ＞ " + "<span>失敗</span>");
            }
            ...
            if (first.getHp() > 0 && latter.getHp() > 0) {
                ...
                if (roll < 6) { //5以下で攻撃に成功した場合(クリティカルだった場合)-------------------------------------
                    log.add(latter.getName() + "1D100<=" + latter.attackSkill() + "【 " + latter.attackName() + " 】"
                            + "(1D100<=" + latter.attackSkill() + ")＞ " + roll + " ＞ "
                            + "<span style='color:#4682b4;'>クリティカル！</span>");
                    ...
                    log.add(latter.getName() + "1D100<=" + latter.attackSkill() + "【 " + latter.attackName() + " 】"
                            + "(1D100<=" + latter.attackSkill() + ")＞ " + roll + " ＞ " + "<span>成功！</span>");
                    ...
                    //回避
                    if (roll < 6) { //回避クリティカル
                        log.add(first.getName() + " : " + "1D100<=" + first.getAvoidance() + "【 回避 】(1D100<="
                                + first.getAvoidance() + ")＞ " + roll + " ＞ "
                                + "<span style='color:#4682b4;'>クリティカル！</span>");
                        ...
                        log.add(first.getName() + " : " + "1D100<=" + first.getAvoidance() + "【 回避 】(1D100<="
                                + first.getAvoidance() + ")＞ " + roll + " ＞ " + "回避成功！");
                    } else {
                        log.add(first.getName() + " : " + "1D100<=" + first.getAvoidance() + "【 回避 】(1D100<="
                                + first.getAvoidance() + ")＞ " + roll + " ＞ " + "回避失敗");
                        //攻撃～気絶判定--------------------------------------------------------
                        DemoBattle.attake(latter, first, latterDb, log);
                    }
                    ...
                    log.add(latter.getName() + "1D100<=" + latter.attackSkill() + "【 " + latter.attackName() + " 】"
                            + "(1D100<=" + latter.attackSkill() + ")＞ " + roll + " ＞ " + "<span>失敗</span>");
                }
